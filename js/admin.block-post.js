...
( function ( $ ) {
	$( function () {
		...
		$( '#block_fields' ).on( 'click', '#block-add-sub-field', function () {
			...
			row.find(
				'.block-fields-edit-control option[value="repeater"]'
			).remove();
		...
		$( '.block-lab-pub-section .edit-post-types' ).on(
			'click',
			function () {
				...
				for ( const postType of excludedPostTypes ) {
					$(
						'.post-types-select-items input[value="' +
							postType +
							'"]'
					).prop( 'checked', false );
				}
		...
		$( '.block-lab-pub-section .save-post-types' ).on(
			'click',
			function () {
				const checked = $(
						'.post-types-select-items input:not(:checked)'
					),
					postTypes = [];
		...
		$( '#block_properties .block-properties-icon-select span' ).on(
			'click',
			function () {
				const svg = $( 'svg', this ).clone();
				$(
					'#block_properties .block-properties-icon-select span.selected'
		...
		$( '#block_properties .block-properties-category' ).on(
			'change',
			function () {
				if ( '__custom' === $( this ).val() ) {
					$( this )
						.next( '.block-properties-category-custom' )
						.css( 'display', 'block' );
				} else {
					$( this )
						.next( '.block-properties-category-custom' )
		...
		$( '#block_template .template-location a.filename' ).on(
			'click',
			function ( event ) {
		...
		$( '#block_template .template-location .click-to-copy input' ).on(
			'blur',
			function () {
		...
		$( '.block-fields-rows' )
			.on( 'click', '.block-fields-actions-delete', function () {
				...
				if (
					0 ===
					$( '.block-fields-rows' ).children( '.block-fields-row' )
						.length
				) {
				...
				if (
					0 !== subRows.length &&
					0 === $( '.block-fields-row', subRows ).length
				) {
			...
			.on( 'click', '.block-fields-actions-duplicate', function () {
				...
				if ( newRow.hasClass( 'block-fields-row-active' ) ) {
					row.find( '.block-fields-actions-edit' )
						.eq( 0 )
						.trigger( 'click' );
				} else {
					newRow
						.find( '.block-fields-actions-edit' )
						.eq( 0 )
						.trigger( 'click' );
				}
			...
			.on(
				'click',
				'.block-fields-actions-edit, a.row-title',
				function () {
					...
					if ( ! currentRow.hasClass( 'block-fields-row-active' ) ) {
						const editRow = $(
							'.block-fields-rows .block-fields-edit'
						);
						...
						$(
							'.block-fields-rows .block-fields-row-active'
						).removeClass( 'block-fields-row-active' );
					}

					currentRow.toggleClass( 'block-fields-row-active' );
					currentRow
						.find( '.block-fields-edit' )
						.first()
						.slideToggle();
					...
					if (
						$( this )
							.closest( '.block-fields-row' )
							.hasClass( 'block-fields-row-active' )
					) {
						const fieldRow = $( this ).closest(
							'.block-fields-row'
						);
						if (
							0 ===
							fieldRow.find( '.block-fields-edit-settings' )
								.length
						) {
							const fieldControl = fieldRow
								.find( '.block-fields-edit-control select' )
								.val();
			...
			.on(
				'click',
				'.block-fields-edit-actions-close a.button',
				function () {
			...
			.on( 'change', '.block-fields-edit-control select', function () {
				...
				if ( 'repeater' === $( this ).val() ) {
					...
					blockFieldSubRowsInit(
						$( '.block-fields-sub-rows', fieldRow )
					);
				} else {
					$(
						'.block-fields-sub-rows,.block-fields-sub-rows-actions',
						fieldRow
					).remove();
				}
			} )
			.on(
				'change',
				'.block-fields-edit-location-settings select',
				function () {
					blockFieldWidthInit(
						$( this ).closest( '.block-fields-row' )
					);
				}
			)
			.on( 'change keyup', '.block-fields-edit-label input', function () {
				...
				if ( 'false' !== slug.data( 'autoslug' ) ) {
					slug.val( slugify( $( this ).val() ) ).trigger( 'change' );
				}
			} )
			...
			.on(
				'mouseenter',
				'.block-fields-row div:not(.block-fields-edit,.block-fields-sub-rows,.block-fields-sub-rows-actions)',
				function () {
	...
	const blockFieldInit = function () {
		if (
			0 ===
			$( '.block-fields-rows' ).children( '.block-fields-row' ).length
		) {
			$( '.block-no-fields' ).show();
		}
	...
	const blockFieldWidthInit = function ( fieldRow ) {
		const widthSettings = fieldRow.find(
				'.block-fields-edit-width-settings'
			),
			locationSettings = fieldRow.find(
				'.block-fields-edit-location-settings'
			);
	...
	const blockPostTypesInit = function () {
		...
		for ( const input of inputs ) {
			...
			if ( -1 === excludedPostTypes.indexOf( postType ) ) {
				displayList.push( $( input ).next( 'label' ).text() );
			}
		}
	...
	const fetchFieldSettings = function ( fieldRow, fieldControl ) {
		...
		wp.ajax.send( 'fetch_field_settings', {
			success( result ) {
				...
				$( '.block-fields-edit-control', fieldRow ).after(
					settingsRows
				);
	...
	const cloneRow = function ( row, append = true ) {
		...
		subRows.each( function () {
			$( this )
				.find( 'input[name^="block-fields-parent"]' )
				.val( newUid );
			newRow
				.find( '.block-fields-sub-rows' )
				.append( cloneRow( $( this ), false ) );
		} );
	...
	const incrementRow = function ( row ) {
		const label = row.find( '.block-fields-edit-label input' ).eq( 0 ),
			...
			matchedNames = $( 'input[name^="block-fields-name"]' ).filter(
				function () {
